...
class DetectionObj(object):
    ...
    def detect(self, images, annotate_on_image=True):
        ...
        for image in images:
            ...
            if annotate_on_image: # tell the script to visualize the bounding box and the
                                  # annotation directly
    ...
    """
    Annotation are prepared and written to disk by the 'serialize_annotation'
    functions, which will create single JSON files containing,
    for each image, the dta regarding detected classes, vertices of the bounding boxes, 
    ...
    '''
    The function get_time conveniently transforms the actual time into a string that can be used in 
    a filename
    '''
    def get_time(self):
        """
    ...
    '''
    Prepare 3 detections pipelines for images, videos, and webcams
    The pipepline for images upload each image into a list
    The pipeline for video lets the VideoFileClip module from moviepy do all the 
    heavy lifting after simply passing the 'detect' function wrapped up in the
    'annotate_photogram' function
    Finally, the pipeline for webcam capture, relies on a simple capture_webcam funtion,
    that, based on OPENCV's Video Capture, records a number of snapshots from the webcam
    returning just the last
    '''
    ...
    '''
    The capture_webcam function will acquite an image from your webcam using cv2.VideoCapture.
    As webcam first have to adjust to the light conditions, few initial shots will be discarded.
    '''
    ...
    '''
    The file_pipeline comprised all the steps necessary to load images
    from storage and visualize/annotate them
    1. Loading images from disk
    2. Apply object detection on the loaded images
    3. Writting annotation for each images in JSON file
    4. If required by Boolean: visualize, represent the images with its bounding boxes
    ...
    '''
    The webcam_pipeline arranges all the steps to annotate image:
    1. Cature image from webcam
    2. Saves captured image to disk
    3. Apply object_detection to image
    4. Saves the annotation JSON file
    5. Represent visually the image with bounding boxes
    '''
